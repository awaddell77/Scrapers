...
def print_u(x):
    try:
        ...
        print("Encountered Unicode Error")

if __name__ == "__main__":
    link, name = '', ''
    link = input("Link: ")
    name = input("Card Name: ")
    while not name or not link:
        print("Please try again")
        link = input("Link (e.g. http://yugioh.wikia.com/wiki/Spirit_Warriors): ")
        name = input("Card Name: ")
    try:
        main(ygo_link_grab(link, name), 1)
    except:
        #sets cell param to 5 in ygo_link_grab function if there is an error
        #need to be more specific
        main(ygo_link_grab(link, name, 5), 1)
    else:
