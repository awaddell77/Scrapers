...
def main_s(x):
    ...
    need_list = ['Product Name:', 'Sku', 'UPC each:', 'Barcode:', 'Release Date:', 'Manufacturer:', 'Product Image:', 'Image Link:', 'Category:']
...
def main_batch1(x):#links must be pre-scraped and saved in a text file
    need_list = ['Product Name', 'Sku', 'UPC each:', 'Release Date:', 'Manufacturer:', 'Product Image:', 'Image Link:','Category:']
...
def gts_split(x):
    ...
    d['Product Name:'] = S_format(contents_head.h1.text).encoder()
    table = x.find('ul', {'class', 'detail_info'})
    contents = table.find_all('li')
    for i in range(0, len(contents)):
        if contents[i].span != None:
            d[S_format(contents[i].span.text).encoder()] = S_format(contents[i].text).encoder()
...
def dict_sort(x):#parses values in order to make them readable
    ...
    n_values = []
    d2 = x
    no_sort = ['Product Name:','Product Image:', 'Image Link:']
    need_list = ['Sku', 'UPC each:', 'Release Date:', 'Manufacturer:', 'Product Image:', 'Image Link:']
    for i in range(0, len(d2.keys())):
        if ':' in d2[d2.keys()[i]] and d2.keys()[i] not in no_sort:
            d2[d2.keys()[i]] = d2[d2.keys()[i]].split(':')[1].strip(' ')
        elif ':' not in d2[d2.keys()[i]] and d2.keys()[i] not in no_sort:#changed
            d2[d2.keys()[i]] = d2[d2.keys()[i]].split(' ')[1]
...
if len(sys.argv) > 1:
    if sys.argv[1] == '-l': #lower case L
        main_batch1(links_1(sys.argv[2]))
    elif sys.argv[1] == '-t':
