...
class MtgParse:
	...
	def normalize(self):
		for i in self.set:
			...
			i['card_number'] = i['number']
			if not i.get('manaCost', ''): i['manaCost'] = ''
			i['manaCost'] = i['manaCost'].replace('{','').replace('}', '')
			if 'Basic Land' in i['type']: i['name'] = i['name'] + ' (' + i['number'] + ')'

		maxlen = len(self.set)
		for i in range(maxlen-1, -1, -1):
			if self.set[i]['number'].isalnum() and 'a' in self.set[i]['number']:
	...
	def combine(self, card_1, card_2):
		...
		card_1['text'] = card_1['text'] + ' // ' + card_2['text']
